...
DATA_PATH = os.getenv(
    "NOUS_DATA_PATH", os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
)
API_PORT = int(os.getenv("NOUS_API_PORT", "5000"))
...
def load_model():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Model loaded", "model_info": info})
...
def generate_response():
    ...
    def generate():
        try:
            for token in model_interface.generate_stream(
                formatted_prompt, max_tokens, temperature, top_k
            ):
...
def start_training():
    ...
    try:
        ...
        if training_interface.is_training and (
            not training_interface.training_thread
            or not training_interface.training_thread.is_alive()
        ):
...
def train_bpe_tokenizer():
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "message": "BPE Tokenizer trained successfully",
                "path": output_path,
            }
        )
    except Exception as e:
...
def load_bpe_tokenizer():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "messsage": f"Loaded BPE tokenizer from {tokenizer_path}"}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def set_tiktoken():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f"Set tokenizer to TikToken's {name}"}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def create_dataset_v2():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "output_path": output_path, "examples": len(all_examples)}
        )
    ...
    try:
        ...
        for file in files:
            ...
            if not (file.filename.endswith(".txt") or file.filename.endswith(".pkl")):
                return jsonify(
                    {
                        "error": f"Invalid file type: {file.filename}. Only .txt and .pkl files are allowed."
                    }
                ), 400
...
def tokenize_dataset():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "output_path": output_path, "num_tokens": len(token_ids)}
        )
            ...
            for i, ids in enumerate(token_ids[:num_examples]):
                if isinstance(ids, list):
                    preview_examples.append(
                        f"Token IDs (length {len(ids)}): {ids[:50]}{'...' if len(ids) > 50 else ''}"
                    )
                else:
...
def remove_dataset():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Dataset removed successfully"})
...
if __name__ == "__main__":
    ...
    print(f"Server running at http://127.0.0.1:{API_PORT}")
