...
class MultimodalTrainer(Trainer):
    ...
    def count_parameters(self):
        ...
        # Patch embeddings
        param_counts['vit'] += self.vit_encoder.patch_embedding.projection.size
        param_counts['vit'] += self.vit_encoder.patch_embedding.cls_token.size
        param_counts['vit'] += self.vit_encoder.patch_embedding.positional_embeddings.size

        # ViT Transformer Blocks
        for block in self.vit_encoder.transformer_stack.blocks:
            # Attention
            param_counts['vit'] += block.attention_layer.W_Q.size
            param_counts['vit'] += block.attention_layer.W_K.size
            param_counts['vit'] += block.attention_layer.W_V.size
            param_counts['vit'] += block.attention_layer.W_O.size

            # FFN/MoE
            if block.use_moe:
                param_counts['vit'] += block.moe.count_params()
            else:
                param_counts['vit'] += block.ffn.W1.size
                param_counts['vit'] += block.ffn.B1.size
                param_counts['vit'] += block.ffn.W2.size
                param_counts['vit'] += block.ffn.B2.size

            # LN params
            param_counts['vit'] += block.gamma_1.size
            param_counts['vit'] += block.beta_1.size
            param_counts['vit'] += block.gamma_2.size
            param_counts['vit'] += block.beta_2.size

        # ViT final LN
        param_counts['vit'] += self.vit_encoder.final_gamma.size
        param_counts['vit'] += self.vit_encoder.final_beta.size
