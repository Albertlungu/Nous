...
class Trainer:
    ...
    def __init__(
        ...
        if self.use_multi_gpu and self.num_devices > 1:
        ...
        # Streaming mode - no token_ids needed
        self.token_ids = None
        ...
        self.optimizer = AdamNested(
            lr=lr, beta1=0.9, beta2=0.95, epsilon=1e-8, min_lr=min_lr
        )
    ...
    def _unflatten_params(self, params):
        ...
        self.embedding_layer.positional_encodings = embeddings_dict[
    ...
    def _replicate_params_to_devices(self):
        ...
        # Replicate each parameter array across devices
        replicated_params = tree.tree_map(
            lambda p: jnp.stack([p] * self.num_devices), params
        )

        return replicated_params

    def _replicate_optimizer_state(self):
        """Replicate optimizer state across all devices for pmap."""
        replicated_m = tree.tree_map(
            lambda p: jnp.stack([p] * self.num_devices), self._adam_m
        )
        replicated_v = tree.tree_map(
            lambda p: jnp.stack([p] * self.num_devices), self._adam_v
        )
        return replicated_m, replicated_v

    def _unreplicate_params(self, replicated_params):
        """Take first device's copy of parameters."""
        return tree.tree_map(lambda p: p[0], replicated_params)

    def _reshape_batch_for_devices(self, batch: np.ndarray):
        """
        Reshape batch to (num_devices, per_device_batch_size, seq_len).
        Batch must be divisible by num_devices.
        """
        batch_size, seq_len = batch.shape
        per_device_batch = batch_size // self.num_devices

        if batch_size % self.num_devices != 0:
            raise ValueError(
                f"Batch size {batch_size} must be divisible by number of devices {self.num_devices}"
            )
    ...
    def train(
        self,
        data_loader: "JAXStreamingLoader",
        epochs: int,
        checkpoint_path: str,
        save_every=1,
        prompt="",
        ...
        Args:
            data_loader (JAXStreamingLoader): Data loader for streaming batches.
        ...
        estimated_batches = data_loader.estimate_total_batches(
        ...
        if self.use_lr_schedule:
            ...
            print("Learning rate schedule enabled:")
        ...
        try:
            for epoch in range(epochs):
                ...
                # Stream batches from data loader
                batch_iterator = data_loader.stream_batches()
                ...
                for batch_idx, batch in enumerate(
                    ...
                    if accumulated_grads is None:
                        ...
                        # Add gradients
                        accumulated_grads = tree.tree_map(
                    ...
                    # Update parameters only when accumulated enough
                ...
                if (
                    prompt and (epoch + 1) % 5 == 0
                ):  # Run generate every 5 epochs - avoids 100s delay every epoch.
    ...
    def print_model_summary(self):
        ...
        print(
            f"Device Type:          {devices[0].device_kind if devices else 'Unknown'}"
        )
        print("-" * 60)
    ...
    def _generate_metadata(self):
        ...
        # Dataset statistics not available in streaming mode
        ...
        metadata = {
            ...
            "training_data": {
                ...
                "total_tokens": "unknown (streaming)",
                "avg_tokens_per_example": "unknown (streaming)",
                "min_tokens": "unknown (streaming)",
                "max_tokens": "unknown (streaming)",
            },
            "training_config": {
            ...
            "training_history": {
                ...
                "losses": self.training_history["losses"][-50:]
                if self.training_history["losses"]
                else [],  # Last 10 losses
                "learning_rates": self.training_history["learning_rates"][-50:]
    ...
    def load_checkpoint(self, path: str):
        ...
        if "training_history" in checkpoint:
            ...
            print(
                f"Loaded training history: {self.training_history['epochs_completed']} epochs completed"
            )
        else:
            print(
                "Warning: No training history in checkpoint. Initializing new history."
            )
            self.training_history = {
                "losses": [],
                "learning_rates": [],
    ...
    def generate(
        ...
        with jax.default_device(jax.devices()[0]):
            for step_idx in range(max_length):
                ...
                if debug:
                    print(
                        f"DEBUG: Generated token {len(token_ids) - prompt_length + 1}: {next_token} (prob: {probs_np[next_token]:.4f})"
                    )
                    top_5_indices = np.argsort(probs_np)[-5:][::-1]
                    print(
                        f"DEBUG: Top 5 tokens: {[(i, probs_np[i]) for i in top_5_indices]}"
                    )
                ...
                if next_token == self.tokenizer.eos_token_id:
                    if debug:
                        print(
                            f"DEBUG: Hit EOS token at position {len(token_ids) - prompt_length}"
                        )
                    break
